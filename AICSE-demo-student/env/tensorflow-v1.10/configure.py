...
def set_mlu_home(environ_cp):
  """Set NEUWARE_HOME."""
  default_mlu_home = '/usr/local/neuware'

  ask_mlu_home = ('Please specify the CNML folder. [Default is %s]: '
                 ) % default_mlu_home

  mlu_home = get_from_env_or_user_or_default(environ_cp, 'NEUWARE_HOME',
                                               ask_mlu_home, default_mlu_home)

  if os.path.exists(os.path.join(mlu_home, 'lib64')) and os.path.exists(
        os.path.join(mlu_home, 'include')):
    environ_cp['NEUWARE_HOME'] = str(mlu_home)

  else:
    raise ValueError('Invalid path to the library folder. %s or %s cannot be found' %
          (os.path.join(mlu_home, 'lib'),
           os.path.exists(os.path.join(mlu_home, 'include'))))



#  """Set CNRT_HOME"""
#  default_mlu_home = '/usr/local/lib/cnrt'
#
#  ask_mlu_home = ('Please specify the CNRT folder. [Default is %s]: '
#                 ) % default_mlu_home
#
#  mlu_home = get_from_env_or_user_or_default(environ_cp, 'CNRT_HOME',
#                                               ask_mlu_home, default_mlu_home)
#
#  if os.path.exists(os.path.join(mlu_home, 'lib')) and os.path.exists(
#        os.path.join(mlu_home, 'include')):
#    environ_cp['CNRT_HOME'] = str(mlu_home)
#  else:
#    raise ValueError('Invalid path to the CNRT folder. %s or %s cannot be found' %
#          (os.path.join(mlu_home, 'lib'),
#           os.path.exists(os.path.join(mlu_home, 'include'))))



def set_other_mlu_vars(environ_cp):
  """Set other library related variables."""
  # Link the CNML header files
  cnml_home = environ_cp.get('NEUWARE_HOME')
  cnrt_home = environ_cp.get('NEUWARE_HOME')
  tensorflow_home = environ_cp.get('TENSORFLOW_HOME')
  cnplugin_home = environ_cp.get('NEUWARE_HOME')

  if os.path.exists(os.path.join(cnml_home, 'include/cnml.h')) and os.path.exists(os.path.join(cnrt_home,'include/cnrt.h')) \
          and os.path.exists(os.path.join(cnplugin_home, 'include/cnplugin.h')):
    if os.path.exists(os.path.join(tensorflow_home, 'third_party/mlu/include/')):
      symlink_force(os.path.join(cnml_home, 'include/cnml.h'), os.path.join(tensorflow_home, 'third_party/mlu/include/cnml.h'))
      symlink_force(os.path.join(cnplugin_home, 'include/cnplugin.h'), os.path.join(tensorflow_home, 'third_party/mlu/include/cnplugin.h'))
      if environ_cp.get('TF_NEED_OFFLINE') == '1':
        symlink_force(os.path.join(cnrt_home, 'include/cnrt.h'), os.path.join(tensorflow_home, 'third_party/mlu/include/cnrt.h'))
    else:
      os.mkdir(os.path.join(tensorflow_home, 'third_party/mlu/include/'))
      symlink_force(os.path.join(cnml_home, 'include/cnml.h'), os.path.join(tensorflow_home, 'third_party/mlu/include/cnml.h'))
      symlink_force(os.path.join(cnplugin_home, 'include/cnplugin.h'), os.path.join(tensorflow_home, 'third_party/mlu/include/cnplugin.h'))
      if environ_cp.get('TF_NEED_OFFLINE') == '1':
        symlink_force(os.path.join(cnrt_home, 'include/cnrt.h'), os.path.join(tensorflow_home, 'third_party/mlu/include/cnrt.h'))
  else:
    raise ValueError('Cannot find the library header file in %s/include or %s/include' %(cnml_home,cnrt_home))

  if os.path.exists(os.path.join(cnml_home, 'lib64/libcnml.so')) and os.path.exists(os.path.join(cnrt_home,'lib64/libcnrt.so')) \
          and os.path.exists(os.path.join(cnplugin_home, 'lib64/libcnplugin.so')):
    if os.path.exists(os.path.join(tensorflow_home, 'third_party/mlu/lib/')):
      symlink_force( os.path.join(cnml_home, 'lib64/libcnml.so'), os.path.join(tensorflow_home, 'third_party/mlu/lib/libcnml.so'))
      symlink_force( os.path.join(cnml_home, 'lib64/libcnplugin.so'), os.path.join(tensorflow_home, 'third_party/mlu/lib/libcnplugin.so'))
      if environ_cp.get('TF_NEED_OFFLINE') == '1':
        symlink_force(os.path.join(cnrt_home, 'lib64/libcnrt.so'), os.path.join(tensorflow_home, 'third_party/mlu/lib/libcnrt.so'))
    else:
      os.mkdir(os.path.join(tensorflow_home, 'third_party/mlu/lib/'))
      symlink_force(os.path.join(cnml_home, 'lib64/libcnml.so'), os.path.join(tensorflow_home, 'third_party/mlu/lib/libcnml.so'))
      symlink_force( os.path.join(cnml_home, 'lib64/libcnplugin.so'), os.path.join(tensorflow_home, 'third_party/mlu/lib/libcnplugin.so'))
      if environ_cp.get('TF_NEED_OFFLINE') == '1':
        symlink_force(os.path.join(cnrt_home, 'lib64/libcnrt.so'), os.path.join(tensorflow_home, 'third_party/mlu/lib/libcnrt.so'))
  else:
    raise ValueError('Cannot find the library library file in %s/lib or %s/lib' % (cnml_home,cnrt_home))
  if os.path.exists(os.path.join(cnml_home, 'lib64/libipu_smmu.so')):
    symlink_force(
        os.path.join(cnml_home, 'lib64/libipu_smmu.so'), os.path.join(tensorflow_home, 'third_party/mlu/lib/libipu_smmu.so'))
...
def main():
  ...
  set_build_var(environ_cp, 'TF_NEED_MLU', 'LIB', 'using_mlu', False, 'mlu')
  if environ_cp.get('TF_NEED_MLU') == '1':
    set_mlu_home(environ_cp)
    set_other_mlu_vars(environ_cp)
