...
prepare_neuware() {
 if [ ${BUILD_RELEASE} == "OFF" ]; then
   ...
   echo "-- use source for release : ${SOURCE_PATH}"
...
do
    ...
    # The following ops will be compiled in both MLU220 and MLU270 mode
    ...
    then
      cncc -O3 ${file} -DFLOAT_MODE=${FLOAT_MODE} --bang-mlu-arch=MLU270 -o \
      ${BUILD_PLUGINOP_DIR}/${file_base_name}_MLU270.o
      cncc -O3 ${file} -DFLOAT_MODE=${FLOAT_MODE} --bang-mlu-arch=MLU220 -o \
      ${BUILD_PLUGINOP_DIR}/${file_base_name}_MLU220.o
    elif [[ $file_base_name =~ "arange" ||
          $file_base_name =~ "init" ||
          $file_base_name =~ "nms" ]]
    then
      cncc -O3 ${file} -DFLOAT_MODE=0 --bang-mlu-arch=MLU270 -DBANG_LOG=${BANG_LOG} -o \
      ${BUILD_PLUGINOP_DIR}/${file_base_name}_MLU270_half.o
      cncc -O3 ${file} -DFLOAT_MODE=1 --bang-mlu-arch=MLU270 -DBANG_LOG=${BANG_LOG} -o \
      ${BUILD_PLUGINOP_DIR}/${file_base_name}_MLU220_float.o
    else
    cncc -O3 ${file} --bang-mlu-arch=${TARGET_MLU_ARCH} -o \
...
g++ ${MAKE_FLAG} -shared -fPIC ${cc_files} -DFLOAT_MODE=${FLOAT_MODE} ${BUILD_PLUGINOP_DIR}/*.o -L ${LIB_PATH} -I ${HEAD_PATH} \
    -I ./common/include  -lcnml -lcnrt --std=c++11 -o ${BUILD_DIR}/libcnplugin.so

if [ ${BUILD_MODE} == "release" ]; then
  strip -s ${BUILD_DIR}/libcnplugin.so
fi
